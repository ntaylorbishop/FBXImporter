...
vec4 get_pixel(vec2 coords, float dx, float dy) {
 return texture2D(gTexDiffuse,coords + vec2(dx, dy));
}

// returns pixel color
float IsEdge(in vec2 coords){
  float dxtex = 1.0 /float(textureSize(gTexDiffuse,0)) ;
  float dytex = 1.0 /float(textureSize(gTexDiffuse,0));
  float pix[9];
  int k = -1;
  float delta;
...
void main(void)
{
    vec4 colorOrg = texture2D( gTexDiffuse, passUV0 );
    vec3 vHSV =  RGBtoHSV(colorOrg.r,colorOrg.g,colorOrg.b);
    vHSV.x = nearestLevel(vHSV.x, 0);
    vHSV.y = nearestLevel(vHSV.y, 1);
    vHSV.z = nearestLevel(vHSV.z, 2);
    float edg = IsEdge(passUV0);
